...
class Course:
    ...
    def __repr__(self) -> str:
        return f"{self.label} ({self.__len__()}/{len(self.target) if self.target else 0})"
...
class CourseBook():
        ...
        if transforms:
            if callable(transforms):
                ...
                if len(transforms) == len(courses):
                    ...
                    raise ValueError("Sequences of Courses and Transforms must have the same length")
    ...
    def __add__ (self, op):
        if isinstance(op, Course):
            ...
            return CourseBook(list(set([v for k, v in self.__courses.items()] + [v for k, v in op.courses.items()])))
        else:
            raise TypeError("class <FullCourse> does not support addtion with types other than 'Course' or 'FullCourse'.")

    def __repr__(self) -> str:
        return '\n'.join([f"{k} : {{ length: {len(course)}, target: {'exists' if course.has_target() else 'none'} }}" for k, course in self.__courses.items()])
